...
wall line at the USB end (the receptacle overhangs the board there) and the plate's rim is cut away across the
whole window at that end (the wall's notch below stays 12 mm), so the port is open from above rather than hidden
under a bridge and no stub of rim sits over the board's corners (`usb_plate_slot`); the window has 2 mm of clearance on the other sides;
