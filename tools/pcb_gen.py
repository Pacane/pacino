...
ARGV = [a for a in sys.argv[1:] if not a.startswith("--variant") and a != "--no-render"]
...
every connection, and names the nets the way the ZMK shield names them.
    ...
    tools/pcb_gen.py                 # both boards, with gerbers and renders
    tools/pcb_gen.py --variant=%(key)s  # just this one
    tools/pcb_gen.py --no-render     # skip the raytracing while iterating

![%(name)s](%(name)s_iso.png)
...
Plus, from the case: 8 x M2x3x3.5 heat-set inserts, 8 x M2x10 screws, 8 x 10 mm bumpons.

## Why the mounting holes are notches

Five of the eight are half-circles in the edge rather than holes through the board; the other three
(the thumb end, the bay's bottom corner and the one between the columns) are real holes. That follows
from the case: five of its screw bosses sit 0.5 mm inside the inner wall line so they merge into the
wall, and the board is the cavity less 0.5 mm -- so those screws' centres land exactly on the board's
edge. The other three bosses stand 2.5-3 mm clear of the wall, because the wall keeps rising past the
bosses in this build and an insert hole at the wall line would be tucked under it; the five that stay
on the line get a 4 mm channel up the inside of the wall instead, so their inserts and screws drop in.
...
VIEWS = (("top",    ["--side", "top",    "--zoom", "1.0"]),
         ("bottom", ["--side", "bottom", "--zoom", "1.0"]),
         ("iso",    ["--rotate=325,0,20", "--perspective", "--floor", "--zoom", "1.15"]))

def render(path, outdir, name):
    """3D renders of the finished board, from the same file the fab gets -- so what the README shows
    is never a hand-made picture of an older revision.  Skipped by --no-render (raytracing is slow)."""
    cli = kicad_cli()
    if not cli: return []
    out = []
    for tag, args in VIEWS:
        f = os.path.join(outdir, "%s_%s.png" % (name, tag))
        r = subprocess.run([cli, "pcb", "render", "--output", f, "--width", "1100", "--height", "950",
                            "--quality", "high", "--background", "opaque", *args, path],
                           capture_output=True, text=True)
        if r.returncode:
            print("  render failed (%s): %s" % (tag, r.stderr.strip()[:120])); return out
        out.append(f)
    return out
...
def one(key):
    ...
    imgs = render(path, out, v["name"]) if RENDER else []
    print("  %-5s %-28s %2d nets, %3d tracks, %2d vias, %d footprints%s%s"
          % (key, os.path.relpath(path, ROOT), len(g.nets), g.tracks, g.vias, len(libs),
             "  gerbers" if z else "", "  %d renders" % len(imgs) if imgs else ""))
